...
class StartPage(tk.Frame):
    ...
    def updatelabels(self):
        ...
        if self.timerflag:
            lapsed=self.timertotal-(time.time()-self.tp1)/60/60
            if lapsed < 0:
                self.timerflag=False
            else:
        ...
        self.update()
    def update(self):
        self.updatetimer=Timer(0.1, self.updatelabels)
        self.updatetimer.start()
        ##print("updating")
    def time(self):
        self.timerflag = True
        self.tp1=time.time()
        mini = eval(self.timernumber.get())
        self.ob.gui_set_timer(mini)
        #print ("time stuff here {} minutes".format(mini))
        self.timertotal=mini/60
